...
#Reading log data folder locations
...
#Combine files from each folder into 1 data frame

library('dplyr',warn.conflicts=FALSE)

#Defining combine function to merge folders
...
#Declaring a,b,c,d variables to hold merged log data
...
columnstodrop<-c('Seg_.','Stall_Dur','Rep_Level','Buff_Level','Del_Rate','Act_Rate')
...
e_same1<-NULL
e_same1<-e[-c(seg_dur_4_remove_ind,seg_dur_6_remove_ind,seg_dur_8_remove_ind),]
...
library('varhandle',warn.conflicts=FALSE))
...
logistic_regression_model<-glm(seg_dur ~ ., data = gtrain,family=binomial(link=logit))
logistic_regression_probs<-predict(logistic_regression_model,newdata = gtest,type='response')
#converting probabilities to class labels
logistic_regression_predictions=rep(4,nrow(gtest))
logistic_regression_predictions[logistic_regression_probs>.5]=10
logistic_regression_accuracy[v]=mean(logistic_regression_predictions==gtestlabel)

#classification tree
	
library('party',warn.conflicts=FALSE)
ctree_model<- ctree(seg_dur ~ ., data = gtrain,controls=ctree_control(minsplit=30,minbucket=10,maxdepth=5))
